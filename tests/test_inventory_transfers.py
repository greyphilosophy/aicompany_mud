...
@pytest.mark.parametrize(
    "lock_target,lock", [("holder", "get_from:false()"), ("item", "get:false()")]
)
...
def test_failed_pickup(transfer, monkeypatch, failure):
    ...
    monkeypatch.setattr(
        w.voice,
        "at_pre_get" if failure == "hook" else "move_to",
        lambda *a, **kw: False,
    )
    run_get(w, "Voice from Nova")
    assert w.voice.location == w.npc
...
def test_pickup_hook_cannot_move_item_from_changed_source(transfer, monkeypatch):
    w = transfer

    def change_source(caller):
        w.voice.move_to(w.other)
        return True

    monkeypatch.setattr(w.voice, "at_pre_get", change_source)
    run_get(w, "Voice from Nova")
    assert w.voice.location == w.other


def test_task_assignment_follows_player_command(transfer):
    w = transfer
    task = w.npc.create_task("Find the key")
    task.key = "Note"
    run_get(w, "Note from Nova")
    assert task.location == w.player
    assert task.db.assignee == w.player


def test_missing_item_leaves_inventory_unchanged(transfer):
    w = transfer
    run_get(w, "Missing from Nova")
    assert w.voice.location == w.npc
    assert w.player.msg.called
