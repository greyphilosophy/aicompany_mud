...
def test_saved_legacy_paths_reload_without_losing_state(world, legacy, canonical):
    ...
    assert loaded.is_typeclass(legacy, exact=True)
    assert cls.objects.get(pk=object_id) == loaded
    assert cls.objects.all_family().filter(pk=object_id).exists()
    if issubclass(cls, Tool):
        assert Tool.objects.all_family().filter(pk=object_id).exists()
    new = create_object(canonical, key="New equipment", location=world.npc)
    assert new.db_typeclass_path == legacy
    assert cls.objects.filter(pk__in=[object_id, new.id]).count() == 2
...
def test_catalog_and_factory_create_independent_functional_tools(world):
    ...
    fresh = {item["id"]: item for item in list_tool_types()}
    assert "say" in fresh["speaker"]["actions"]
...
@pytest.mark.parametrize(
    "type_id", ["missing", "typeclasses.tools.speaker.Speaker", "", None, []]
)
...
def test_factory_rejects_deleted_holder(world):
    from typeclasses.tools.factory import create_tool
    ...
    count = ObjectDB.objects.count()
    with pytest.raises(ValueError):
        create_tool("speaker", location=holder)
    assert ObjectDB.objects.count() == count


def test_factory_tool_works_through_player_command(world, monkeypatch):
    from typeclasses.characters import Character
    from typeclasses.tools.factory import create_tool

    player = create_object(Character, key="Player", location=world.room)
    create_tool("sticky_note_pad", location=player, key="Notes")
    monkeypatch.setattr(player, "msg", lambda *args, **kwargs: None)
    player.execute_cmd('tool Notes/create_task = {"objective": "Find the key"}')
    assert player.get_active_task().db.objective == "Find the key"


def test_tool_can_end_burst_without_a_concrete_brain_dependency(world):
    from typeclasses.components.brain import Brain

    tool = create_object(HandoffTool, key="Handoff", location=world.npc)
    create_object(Brain, key="Brain", location=world.npc)
    world.npc.create_task("Try the handoff")
    decide(world, {"action": "invoke", "tool": tool.dbref, "name": "handoff"})
    assert tool.db.used
    assert len(world.jobs) == 1
    assert world.npc.ndb.agency_state == "waiting"
